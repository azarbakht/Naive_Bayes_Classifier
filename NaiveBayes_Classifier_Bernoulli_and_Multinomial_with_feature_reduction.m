...
numberOfDocsInCat = zeros(numberOfCategories,1);
...
for i=1: size(numberOfEachWordInCat,2)
    if  min( numberOfEachWordInCat(:,i)) ~=0 
        if ( std(numberOfEachWordInCat(:,i))/mean(numberOfEachWordInCat(:,i)) )<0.6 
         uselessVocabIndex(end +1)=i;
        end
    elseif ( mean (numberOfEachWordInCat(:,i) )~= numberOfEachWordInCat(1,i))
       ImportanVocabIndex(end+1)=i;
    else
        uselessVocabIndex(end +1)=i;
    end
end

numberOfVocabularyNew =size(ImportanVocabIndex,2);

% reduce the fetures of the test dataset
...
% for docIDTestIndex = 300:330%max(docIDTest) % loop over each document
    %docIDTestIndex
   ...
   for i = min(docBounds):max(docBounds)
    ...
    for counterCategory = 1:numberOfCategories
        likelihood = 0;
        for wordIndexNew = 1: size(ImportanVocabIndex,2)%size(numberOfEachWordInDocTest)
            wordIndex = ImportanVocabIndex(wordIndexNew);
...
%  save runResult20140502Time2203WithFeatureReduction;
