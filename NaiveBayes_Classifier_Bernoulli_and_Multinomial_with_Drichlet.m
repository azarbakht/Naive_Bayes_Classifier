...
DirichletParameters=[ 1+0.00001 1+0.0001 1+0.001 1+0.1 1+1];
...
accuracyTotal = zeros(1,size(DirichletParameters,2));

for counterDirichlet = 1: size(DirichletParameters,2)
    ...
    for i = 1: numberOfCategories
        ...
        elseif testMethod == constTestMethod_multinomial
            % with laplacian smoothing
            probEachWordInCat(i,:) = double(numberOfEachWordInCat(i,:) - 1 + DirichletParameters(counterDirichlet)) / double(sumRow + numberOfVocabulary *DirichletParameters(counterDirichlet)-numberOfVocabulary);
        end
    end
    ...
    for docIDTestIndex =315: 330 % loop over each document
%     for docIDTestIndex =1: max(docIDTest) % loop over each document
    ...
    for i = 1: numberOfCategories
        ...
        for k = min(catBounds):max(catBounds)
            if predictedLabel(docIDTest(k,1))>0
    ...
    accuracyTotal(counterDirichlet) = double( sum(accuracyWithinCat))/double( maxVal);
